<?php
// $Id: index.php 17 2007-11-18 14:22:36Z LupusC $
//  ------------------------------------------------------------------------ //
...
if ( empty($poll_id) ) {
	...
	if ( $polls_arr[$i]->hasExpired() &&  $polls_arr[$i]->getVar("mail_status") != POLL_MAILED ) {
    	$polls_mail =& Umfrage::getAll(array('poll_id='.$polls['pollId']), true, 'weight ASC, end_time DESC');
	...
	$digits = 1 + floor(log10($total)); 
        ...
        foreach ($options_arr as $option) {
          ...
          $mailbody .= sprintf("%" . $digits . "d (%3d%%) " . _PL_MAILBODY2 . "%s\n" , 
          				$option->getVar("option_count"), $percent, $option->getVar('option_text'));
	...
	//election mode
	if($poll->getVars("polltype") != 1) redirect_header(XOOPS_URL."/modules/umfrage/index.php", 2, $msg);
	else redirect_header(XOOPS_URL."/modules/umfrage/pollresults.php?poll_id=$poll_id", 2, $msg);
	exit();
