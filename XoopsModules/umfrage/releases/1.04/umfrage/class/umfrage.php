...
class Umfrage extends XoopsObject {
	...
	function store() {
		...
		if (empty ($poll_id)) {
			...
			$sql = "INSERT INTO ".$this->db->prefix("umfrage_desc")." (poll_id, question, description, user_id, start_time, end_time, votes, voters, display, weight, multiple, mail_status, polltype, autoblockremove) VALUES ($poll_id, ".$this->db->quoteString($question).", ".$this->db->quoteString($description).", $user_id, $start_time, $end_time, 0, 0, $display, $weight, $multiple, $mail_status, $polltype, $autoblockremove)";
		} else {
			$sql = "UPDATE ".$this->db->prefix("umfrage_desc")." SET question=".$this->db->quoteString($question).", description=".$this->db->quoteString($description).", start_time=$start_time, end_time=$end_time, display=$display, weight=$weight, multiple=$multiple, mail_status=$mail_status, polltype=$polltype, autoblockremove=$autoblockremove WHERE poll_id=$poll_id";
		}
		//echo "<br>$sql<br>";
