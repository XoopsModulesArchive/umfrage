...
if ($op == 'list') {
	...
	if (is_array($polls_arr) && $polls_count > 0) {
		...
		for ($i = 0; $i < $max; $i ++) {
			...
			echo "<tr class='bg1'><td align='center'><input type='hidden' name='poll_id[$i]' value='".$polls_arr[$i]->getVar('poll_id') . "'><input type='hidden' name='old_display[$i]' value='" . $polls_arr[$i]->getVar('display') . "'><input type='checkbox' name='display[$i]' value='1'" . $checked . "></td><td><input type='hidden' name='old_weight[$i]' value='" . $polls_arr[$i]->getVar(
                    'weight'
                ) . "'><input type='text' name='weight[$i]' value='" . $polls_arr[$i]->getVar('weight') . "' size='6' maxlength='5'></td><td>" . $polls_arr[$i]->getVar('question') . "</td><td align='center'>" . $polls_arr[$i]->getVar('voters') . "</td><td align='center'>" . $polls_arr[$i]->getVar(
                    'votes'
                ) . '</td><td>'
                 . $end . "</td><td align='right'><a href='index.php?op=edit&amp;poll_id=" . $polls_arr[$i]->getVar('poll_id') . "'>" . _EDIT . "</a><br><a href='index.php?op=delete&amp;poll_id=" . $polls_arr[$i]->getVar('poll_id') . "'>" . _DELETE . "</a><br><a href='index.php?op=log&amp;poll_id=" . $polls_arr[$i]->getVar(
                    'poll_id'
                ) . "'>" . _AM_VIEWLOG . '</a></td></tr>';
		}
